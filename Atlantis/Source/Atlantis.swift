...
public class Atlantis: NSObject {
    ...
    public class Logger: NSObject {
        ...
        fileprivate static func log(_ x: Any?, _ logSettings: LogSettings) {
            // get the type name
            ...
            if let x = x {
                ...
                case (is Bool): break
                // arrays
                case (is NSArray): jsonString = toPrettyJSONString(NSObject.reflect(objects: x as! NSArray)); break
                // dictionaries
                case (is NSDictionary): jsonString = toPrettyJSONString(x) ?? "\n\(x)"; break
                // errors
                case (is NSError):
